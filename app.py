"""
Novi Cloud Backend v3.3
Groq AI + ElevenLabs + Weather + Voice Transcription + Decision Engine
"""
...
def run_decision_engine(user_id, city="Amman"):
    ...
    if sleep_h > 0:
        if sleep_h < 5:
            insights.append(f"Only {sleep_h}h sleep — critically low")
            actions.append("Prioritize sleep tonight, aim for 8 hours")
        elif sleep_h < 7:
            insights.append(f"Sleep slightly below optimal ({sleep_h}h)")
            actions.append("Try sleeping 30 minutes earlier tonight")
        else:
            insights.append(f"Good sleep: {sleep_h}h ✓")

    if water > 0:
        if water < 4:
            insights.append(f"Low hydration: only {water}/8 glasses")
            actions.append("Drink 2 glasses of water right now")
        elif water < 7:
            insights.append(f"Moderate hydration: {water}/8 glasses")
            actions.append("Keep drinking to hit your 8-glass goal")
        else:
            insights.append(f"Great hydration: {water} glasses ✓")

    if exercise == 0:
        actions.append("Even a 10-minute walk can boost your mood and energy")
    else:
        insights.append(f"Active today: {exercise} session(s) ✓")
    ...
    if weather_data:
        ...
        if temp > 33:
            insights.append(f"Intense heat in {city_name}: {temp}°C")
            actions.append("Stay hydrated, avoid outdoor activity 11am–4pm")
        elif temp > 27:
            insights.append(f"Warm day in {city_name}: {temp}°C")
            actions.append("Drink extra water and seek shade outdoors")
        elif condition in ["Rain", "Drizzle", "Thunderstorm"]:
            insights.append(f"Rainy in {city_name} today")
            actions.append("Perfect day for indoor meditation or yoga")
        elif condition == "Clear" and 15 <= temp <= 27:
            insights.append(f"Beautiful weather in {city_name}: {temp}°C ☀️")
            actions.append("Great day for a nature walk — even 20 minutes helps!")
        elif temp < 10:
            insights.append(f"Cold in {city_name}: {temp}°C")
            actions.append("Warm herbal tea and layer up before going out")

    if mood in ["sad", "stressed", "sick"]:
        insights.append(f"Feeling {mood} today")
        actions.append("Be gentle with yourself. Try 5 deep breaths or a brief walk in nature")
    elif mood in ["amazing", "happy", "energetic"]:
        insights.append(f"Great mood: feeling {mood}! 🌟")

    return {
        "weather":  weather_data,
        "insights": insights,
        "actions":  actions,
        "stats":    {"water": water, "sleep": sleep_h, "exercise": exercise, "mood": mood},
    }
...
3. Indirect questions that connect to health/nature → Find the health or nature angle and answer
...
@app.route('/')
def index():
    return jsonify({"status": "Novi server v3.3 — Groq + Weather + Voice 🌿"})

@app.route('/health')
def health():
    return jsonify({"ok": True, "version": "3.3", "ai": "groq"})
...
def chat():
    ...
    if is_crisis(question):
        return jsonify({"answer": "I hear you, and I want you to know your feelings are valid. You don't have to face this alone. Please reach out to a crisis helpline right now — trained people are ready to listen and help. In Jordan you can call 110. You matter, and help is available. 💚", "followups": ["How can nature help with difficult emotions?", "What are some grounding techniques from nature?"]})
    ...
    try:
        ...
        if today_log:
            ...
            if parts:
                health_context = f"\nUSER'S HEALTH DATA TODAY: {', '.join(parts)}. Reference this when relevant to the conversation."

        # Build weather context
        weather_data = get_weather(city)
        weather_context = ""
        if weather_data:
            weather_context = f"\nCURRENT WEATHER in {weather_data['city']}, Jordan: {weather_data['temp']}°C, {weather_data['description']}, humidity {weather_data['humidity']}%. Reference this when relevant."
...
def log_health():
    ...
    if existing:
        ...
        health_logs[user_id].append({"date": today, "water": data.get("water",0), "sleep": data.get("sleep",0), "exercise": data.get("exercise",0), "mood": data.get("mood","")})
    return jsonify({"ok": True})
...
def analyze():
    ...
    if result["weather"]:
        w = result["weather"]
        context_parts.append(f"weather in {w['city']}: {w['temp']}°C, {w['description']}")
...
Write 3-4 warm, specific, actionable sentences. Reference their actual city and weather if available. Sound like a caring friend who truly knows them. {lang}"""
